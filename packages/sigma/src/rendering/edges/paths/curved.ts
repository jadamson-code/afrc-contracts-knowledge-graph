...
float path_curved_length(vec2 source, vec2 target) {
  ...
  float d3 = length(path_curved_derivative(t3, source, target));
...
float path_curved_t_at_distance(float targetDist, vec2 source, vec2 target) {
  if (targetDist <= 0.0) return 0.0;

  float totalLen = path_curved_length(source, target);
  ...
  for (int i = 0; i < 12; i++) {
    ...
    float d0 = length(path_curved_derivative(0.0, source, target));
    float dMid2 = length(path_curved_derivative(mid * 0.5, source, target));
    float dMid = length(path_curved_derivative(mid, source, target));
...
float path_curved_closest_t(vec2 p, vec2 source, vec2 target) {
  // Coarse search
  float bestT = 0.0;
  float bestDist = 1e10;
  ...
  for (int i = 0; i <= 8; i++) {
    float t = float(i) / 8.0;
    vec2 pos = path_curved_position(t, source, target);
  ...
  for (int i = 0; i < 8; i++) {
    ...
    float d1 = length(p - path_curved_position(mid1, source, target));
    float d2 = length(p - path_curved_position(mid2, source, target));
...
float path_curved_distance(vec2 p, vec2 source, vec2 target) {
  // Find closest point on curve using iterative search
  float closestT = path_curved_closest_t(p, source, target);
  vec2 closest = path_curved_position(closestT, source, target);
  vec2 diff = p - closest;
