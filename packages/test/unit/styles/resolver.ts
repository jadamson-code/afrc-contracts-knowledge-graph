...
describe("Style evaluation system", () => {
  describe("evaluateStatePredicate", () => {
    ...
    test("object predicate matches all specified values", () => {
      ...
      expect(
        evaluateStatePredicate({ isHovered: true, isHidden: false }, attrs, state, defaultGraphState, graph),
      ).toBe(true);
    });
  ...
  describe("resolveGraphicValue - literal values", () => {
    ...
    test("returns default for null value", () => {
      expect(resolveGraphicValue(null as unknown as string, attrs, defaultNodeState, defaultGraphState, graph, "#000")).toBe("#000");
    });

    test("returns default for undefined value", () => {
      expect(resolveGraphicValue(undefined as unknown as string, attrs, defaultNodeState, defaultGraphState, graph, "#000")).toBe("#000");
    });
  });
