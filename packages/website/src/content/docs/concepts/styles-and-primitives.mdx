...
:::note
Everything is rendered in the same WebGL context, and sigma still supports the `zIndex` style property. This new `depth` mechanism makes it much easier to handle depth management in interactions. It also allows rendering some edges hover some nodes.
:::
