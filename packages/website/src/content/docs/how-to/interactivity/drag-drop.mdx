...
`autoRescale: "once"` freezes the coordinate system after the first render, preventing the viewport from rescaling as
...
This callback connects your selection state to drag. When the dragged node is selected, all selected nodes move
together:
...
When position attributes differ from `x`/`y` (e.g. via styles mapping `x: { attribute: "lng" }`), sigma infers the
